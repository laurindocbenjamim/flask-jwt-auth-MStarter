...
class UserApi(Resource):
    ...
    def post(self):
        ...
        args["type_of_user"] = "normal"
        args["user_confirmed"] = True

        # Validate the country
        if (str(args.get('countryName', '')).upper() not in [country.upper() for country in current_app.config.get('ALLOWED_COUNTRIES', [])] and
        str(args.get('countryName', '')).upper() not in [code.upper() for code in current_app.config.get('ALLOWED_COUNTRIES_CODE', [])]):
            return make_response(
                jsonify(
                    status_code=400,
                    error=f"Invalid country. Country {args.get('countryName')} not allowed."
                ),
                400
            )
        
        
        status, sms = create_user(new_user=create_user_object(args))
        if not status:
